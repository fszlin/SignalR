    ...
    public class UnityHttpResponse
    {
        ...
        string ReadLine(Stream stream)
        {
            ...
            while (true)
            {
                ...
                if (c == -1)
                {
                    throw new UnityHttpException("Unterminated Stream Encountered.");
                }
                if ((byte)c == UnityHttpRequest.EOL[1])
                    break;
                line.Add((byte)c);
            }
        ...
        public void ReadFromStream(Stream inputStream)
        {
            ...
            if (!int.TryParse(top[1], out status))
                throw new UnityHttpException("Bad Status Code");
            ...
            using (var output = new MemoryStream())
            {
                ...
                if (request.cookieJar != null)
                {
                    ...
                    request.cookieJar.SetCookies(this.request.uri, string.Join(";", cookies.ToArray()));
                }
                ...
                else
                {
                    ...
                    {
                        throw new UnityHttpException("Response length does not match content length");
                    }
                }
